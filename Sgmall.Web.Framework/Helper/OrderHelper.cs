            ...
            var products = skuIds.Select(item =>
           {
               ...
               if (ltmbuy != null)
               {
                   if (count > ltmbuy.LimitCountOfThePeople)
                       throw new SgmallException("超过最大限购数量：" + ltmbuy.LimitCountOfThePeople.ToString() + "");
               }
               if (sku.Stock < count)
               {
               ...
               return new CartItemModel()
               {
                   ...
                   imgUrl = Core.SgmallIO.GetRomoteProductSizeImage(sku.ProductInfo.RelativePath, 1, (int)Sgmall.CommonModel.ImageSize.Size_100),
                   ...
                   color = sku.Color,
                   version = sku.Version,
                   IsSelf = sku.ProductInfo.Sgmall_Shops.IsSelf
               };
           }).ToList();
        ...
        public static List<CartItemModel> GenerateCartItem(IEnumerable<ShoppingCartItem> cartItems)
        {
            var products = cartItems.Select(item =>
           {
               ...
               return new CartItemModel()
               {
                   skuId = item.SkuId,
                   id = product.Id,
                   imgUrl = Core.SgmallIO.GetRomoteProductSizeImage(product.RelativePath, 1, (int)Sgmall.CommonModel.ImageSize.Size_100),
                   ...
                   size = sku.Size,
                   version = sku.Version,
                   IsSelf = product.Sgmall_Shops.IsSelf
               };
           }).ToList();
