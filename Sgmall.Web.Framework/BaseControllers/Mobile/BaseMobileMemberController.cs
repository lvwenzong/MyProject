...
namespace Sgmall.Web.Framework
{
    ...
    public abstract class BaseMobileMemberController : BaseMobileTemplatesController
    {
        ...
        protected bool IsAnonymity()
        {
            ...
            string[] controllers = { "cart", "order","member" };
            string[] actions = { "editbranchproducttocart", "getbranchcartproducts", "clearbranchcartproducts", "clearbranchcartinvalidproducts", "expressinfo", "pushvisit" };
        ...
        bool ProcessInvalidUser_NormalRequest(ActionExecutingContext filterContext)
        {
            bool end = true;
            //处理手动退出后不自动登录
            string actlogout = WebHelper.GetCookie(CookieKeysCollection.Sgmall_ACTIVELOGOUT);
            ...
            if (imobileOauth != null && GetRequestType(filterContext.HttpContext.Request) == Core.PlatformType.WeiXin)//找到了支持的登录接口
            {
                ...
                string redirectUrl;
                //string strShopid = WebHelper.GetCookie(CookieKeysCollection.Sgmall_SHOP);
                ...
                if (string.IsNullOrWhiteSpace(redirectUrl))//待跳转地址为空，说明已经经过了用户授权页面
                {
                    if (userInfo != null && !string.IsNullOrWhiteSpace(userInfo.OpenId))//用户信息不为空并且OpenId不为空，说明用户已经授权
                    {
                        if (AppidType == Model.MemberOpenIdInfo.AppIdTypeEnum.Payment)
                        {
                            ...
                            WebHelper.SetCookie(CookieKeysCollection.Sgmall_USER_OpenID, curMenberOpenId);
                        }
                        //检查是否已经有用户绑定过该OpenId
                        //Sgmall.Core.Log.Debug("InvalidUser LoginProvider=" + userInfo.LoginProvider);
                        //Sgmall.Core.Log.Debug("InvalidUser OpenId=" + userInfo.OpenId);
                        //Sgmall.Core.Log.Debug("InvalidUser UnionId=" + userInfo.UnionId);
                        ...
                        else//未绑定过，则跳转至登录绑定页面
                        {
                            normalLoginUrl = string.Format("/m-{0}/Login/Entrance?openId={1}&serviceProvider={2}&nickName={3}&realName={4}&headimgurl={5}&returnUrl={6}&AppidType={7}&unionid={8}&sex={9}&city={10}&province={11}&country={12}",
                                ...
                                userInfo.OpenId,
                               "Sgmall.Plugin.OAuth.WeiXin",//使用同微信登录插件一致的名称， 以此保证微信信任与微信商城登录用户信息统一
            ...
            if (imobileOauth != null && GetRequestType(filterContext.HttpContext.Request) == Core.PlatformType.WeiXin)//找到了支持的登录接口
            {
                ...
                if (string.IsNullOrWhiteSpace(redirectUrl))//待跳转地址为空，说明已经经过了用户授权页面
                {
                    ...
                    if (userInfo != null && !string.IsNullOrWhiteSpace(userInfo.OpenId))//用户信息不为空并且OpenId不为空，说明用户已经授权
                    {
                        ...
                        //Sgmall.Core.Log.Debug("BindOpenIdToUser LoginProvider=" + userInfo.LoginProvider);
                        //Sgmall.Core.Log.Debug("BindOpenIdToUser OpenId=" + userInfo.OpenId);
                        //Sgmall.Core.Log.Debug("BindOpenIdToUser UnionId=" + userInfo.UnionId);
                        //检查是否已经有用户绑定过该OpenId
                        Model.UserMemberInfo existUser = null;
                        ...
                        {
                            MemberApplication.BindMember(CurrentUser.Id, "Sgmall.Plugin.OAuth.WeiXin", userInfo.OpenId, AppidType, userInfo.Sex, userInfo.Headimgurl, unionid: userInfo.UnionId);
                            //end = false;//不再中断当前action
                        }
                    }
                }
