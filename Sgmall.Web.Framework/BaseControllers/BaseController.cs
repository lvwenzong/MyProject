...
namespace Sgmall.Web.Framework
{
    ...
    public abstract class BaseController : Controller
    {
        ...
        public BaseController()
        {
            ...
            var routeData = curhttp.Request.RequestContext.RouteData;
            //routeData.Values["action"] = "index";
            if (routeData.Values["controller"].ToString().ToLower() == "login" && routeData.Values["action"].ToString().ToLower() == "")
            {
                routeData.Values["action"] = "entrance";
            }
            else if (routeData.Values["controller"].ToString().ToLower() != "login" && routeData.Values["action"].ToString().ToLower() == "")
            {
                routeData.Values["action"] = "index";
            }
            if (!IsInstalled())
        ...
        public void InitJumpUrlRoute()
        {
            ...
            _JumpUrlRouteData.Add(_tmp);
            _tmp = new JumpUrlRoute() { Action = "Index33", Area = "Web", Controller = "Login", PC = @"/login", WAP = @"/login/entrance", WX = @"/login/entrance" };
            _JumpUrlRouteData.Add(_tmp);
        ...
        public List<long> GetDistributionUserLinkId()
        {
            ...
            string _tmp = Core.Helper.WebHelper.GetCookie(CookieKeysCollection.Sgmall_DISTRIBUTIONUSERLINKIDS);
        ...
        {
            Core.Helper.WebHelper.DeleteCookie(CookieKeysCollection.Sgmall_DISTRIBUTIONUSERLINKIDS);
        }
        /// <summary>
        /// 维护分销员
        /// </summary>
        ...
        public void SaveDistributionUserLinkId(long partnerid, long shopid, long uid)
        {
            if (partnerid > 0 && shopid > 0)
            {
                ...
                {
                    //保存cookie
                    Core.Helper.WebHelper.SetCookie(CookieKeysCollection.Sgmall_DISTRIBUTIONUSERLINKIDS, string.Join(",", links.ToArray()));
                }
                else
                {
        ...
        protected override void OnException(ExceptionContext filterContext)
        {
            ...
            if (!(innerEx is SgmallException) && !(innerEx is PluginException))
            {
                ...
                msg = "Thời gian hiệu lực chưa được nhập. Vui lòng điền vào.";/*系统发生错误，请重试，如果再次发生错误，请联系客服！*/
            }
        ...
        protected virtual void SetUserLoginCookie(long userId, DateTime? expiredTime = null)
        {
            ...
            {
                WebHelper.SetCookie(CookieKeysCollection.Sgmall_USER, cookieValue, expiredTime.Value);
                WebHelper.SetCookie("Sgmall_UserId", userId.ToString(), expiredTime.Value);
            }
            else
            {
                WebHelper.SetCookie(CookieKeysCollection.Sgmall_USER, cookieValue);
                WebHelper.SetCookie("Sgmall_UserId", userId.ToString());
            }
        }
        ...
        public static UserMemberInfo GetUser(HttpRequestBase request)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                if (userId != 0)
                {
                    WebHelper.SetCookie(CookieKeysCollection.Sgmall_USER, token);
                    WebHelper.SetCookie("Sgmall_UserId", userId.ToString());
                }
            }
            if (userId == 0)
            {
                var cookieValue = WebHelper.GetCookie(CookieKeysCollection.Sgmall_USER);
            ...
            if (userId != 0)
            {
                ...
                if (siteInfo != null)
                {
                    ...
                    {
                        userInfo.MemberDiscount = siteInfo != null ? Math.Round(siteInfo.DefaultDiscountRate / 100, 2, MidpointRounding.AwayFromZero) : 1.0M;//默认折扣为1（没有折扣）
                    }
        ...
        {
            bool result = false;
            HttpWebResponse urlresponse = Sgmall.Core.Helper.WebHelper.GetURLResponse(url);
            if (urlresponse != null)
            {
